export interface Camera {
  id: string;
  name: string;
  brand: string;
  model: string;
  specs: string[];
  description: string;
  image: string;
  purchaseYear?: number;
  favorite?: boolean;
}

export const cameras: Camera[] = [
  {
    id: "1",
    name: "Nikon ℤ63",
    brand: "Nikon",
    model: "ℤ63",
    specs: [
      "24.5MP partially stacked CMOS sensor",
      "EXPEED 7 processor",
      "6K/60p internal RAW recording",
      "Full HD/240p video recording for slow-motion",
      "Advanced subject detection with 9 subject types",
      "760g weight",
    ],
    description:
      "My primary camera body for both photography and videography. Excellent low-light performance and reliable autofocus system.",
    image: "/images/camera/nikon_z63.jpg",
    purchaseYear: 2025,
    favorite: true,
  },
  {
    id: "2",
    name: "Fujifilm X-T50",
    brand: "Fujifilm",
    model: "X-T50",
    specs: [
      "40.2MP APS-C X-Trans CMOS 5 HR sensor",
      "X-Processor 5",
      "Film Simulation",
      "6.2K video recording",
      "Full HD/240p video recording for slow-motion",
      "Built-in flash",
      "438g weight",
    ],
    description: "My go-to camera for portrait!",
    image: "/images/camera/fujifilm_xt50.jpg",
    purchaseYear: 2025,
    favorite: true,
  },
  {
    id: "3",
    name: "Leica Typ 701",
    brand: "Leica",
    model: "Typ 701",
    specs: [
      "16.2MP APS-C CMOS sensor",
      "3.7-inch touchscreen LCD",
      "Built-in flash",
      "Made in Germany",
      "339g weight",
    ],
    description:
      "I love the camera design, the body is crafted from a single block of aluminum!",
    image: "/images/camera/leica_typ701.jpg",
    purchaseYear: 2023,
    favorite: false,
  },
  {
    id: "4",
    name: "Nikon FM2",
    brand: "Nikon",
    model: "FM2",
    specs: [
      "135 film (35mm) SLR camera",
      "Mechanical Focal-Plane Shutter",
      "Nikon F mount",
      "12-6400 ISO range",
      "1s-1/4000s shutter speed",
      "Flash syncs at 1/250s",
      "540g weight",
    ],
    description:
      "In production from 1982-2001, this camera is one the most reliable film cameras. (I play mirrorless camera more)",
    image: "/images/camera/nikon_fm2.webp",
    purchaseYear: 2023,
    favorite: false,
  },
  {
    id: "10",
    name: "Nikon D5600",
    brand: "Nikon",
    model: "D5600",
    specs: [
      "24.2 MP DX-format CMOS sensor",
      "EXPEED 4 processor",
      "3.2 inch fully articulating touchscreen LCD with 1.04M dots",
      "5.0 fps continuous shooting",
      "465g weight",
    ],
    description:
      "My first DSLR camera! (I later switched to mirrorless full-frame camera, Nikon ℤ5, for better image quality)",
    image: "/images/camera/nikon_d5600.jpg",
    purchaseYear: 2021,
    favorite: false,
  },
  {
    id: "17",
    name: "Chuzhao M1",
    brand: "Chuzhao",
    model: "M1",
    specs: [
      "12MP 1/4-inch CMOS sensor",
      "1080p video",
      "1:1 aspect ratio for square photos",
      "1.54-inch LCD screen",
      "115g weight",
    ],
    description:
      "A fun little TLR-like (Twin-Lens Reflex) toy camera, it has a waist-level viewfinder.",
    image: "/images/camera/chuzhao_m1.webp",
    purchaseYear: 2025,
    favorite: false,
  },
  {
    id: "18",
    name: "Starbucks Retro Digital Camera ",
    brand: "Starbucks",
    model: "Silver and Green",
    specs: [
      "0.3MP CMOS sensor (48MP claimed, questionable)",
      "2.4-inch LCD screen",
      "Compact fill light",
      "Front and rear cameras",
      "9 built-in Y2K-style retro filters",
      "MicroSD card slot",
      "119g weight",
    ],
    description: "A toy camera, not for perfesional photography.",
    image: "/images/camera/starbucks.png",
    purchaseYear: 2026,
    favorite: false,
  },
  {
    id: "19",
    name: "Canon A-1",
    brand: "Canon",
    model: "A-1",
    specs: [
      "135 film (35mm) SLR camera",
      "Full Programmed AE, Shutter-speed priority AE, Aperture-priority AE exposure modes",
      "TTL center-weighted average metering",
      "Flash syncs at 1/60s",
      "Canon FD mount",
      "6–12800 ISO range",
      "Electronically Controlled Focal-Plane Shutter",
      "30s–1/1000s shutter speed",
      "620g weight",
    ],
    description:
      "A beginner friendly film camera, got Aperature priority mode!",
    image: "/images/camera/canon_a1.webp",
    purchaseYear: 2026,
    favorite: false,
  },

  // Add more cameras here as needed
];
