...
- **useState** is a hook that adds state to functional components. It returns an array with the **current state value and a setter function**. When the setter is called, it triggers a re-render with the new state value. setState schedule state change(async)
